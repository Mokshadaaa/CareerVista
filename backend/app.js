...
function findUser(username) {
  return rtcUsers.find((u) => u.username === username);
}
...
wsServer.on("request", (req) => {
  ...
  connection.on("message", (message) => {
    ...
    switch (data.type) {
      case "store_user":
        if (user != null) return;
        rtcUsers.push({ conn: connection, username: data.username });
        console.log("WebRTC user stored:", data.username);
        break;
  ...
  connection.on("close", () => {
    const idx = rtcUsers.findIndex((u) => u.conn === connection);
    if (idx !== -1) rtcUsers.splice(idx, 1);
  });
});
